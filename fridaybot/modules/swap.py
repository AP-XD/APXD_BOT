...
if not os.path.isdir("./dco/"):
    os.makedirs("./dco/")
@bot.on(admin_cmd(pattern=r"swap"))
async def face_swap(event):
    path = "dck"
    reply = await event.get_reply_message()
    lol = await borg.download_media(reply.media, path)
    file_name = "faceswap.jpg"
    hehe = path + "/" + file_name
    ...
    img = cv2.imread("photo_2021-02-11_09-07-34.jpg")
    ...
    cv2.imwrite(file_name, seamlessclone)
