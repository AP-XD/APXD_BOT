...
#@command(outgoing=True, pattern=r"^.ungmuute ?(\d+)?")
@borg.on(admin_cmd(pattern=r"ungmuute ?(\d+)?"))
...
async def watcher(event):
    if is_muted(event.sender_id, "gmuute"):
        await event.delete()
