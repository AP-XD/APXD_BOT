...
from fridaybot.utils import register, errors_handler
...
async def gben(fridaybot):
    ...
    if fridaybot.is_private:
        user = fridaybot.chat
        rank = fridaybot.pattern_match.group(1)
    else:
        fridaybot.chat.title
    try:
        user, rank = await get_full_user(fridaybot)
    except:
        pass
    if me == user:
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await fridaybot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
