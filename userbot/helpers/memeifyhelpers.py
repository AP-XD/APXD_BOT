...
async def cat_meme(topString, bottomString, filename, endname):
    ...
    font = ImageFont.truetype("fridaybot/helpers/styles/impact.ttf", fontSize)
    ...
    while topTextSize[0] > imageSize[0] - \
        ...
        font = ImageFont.truetype(
            "fridaybot/helpers/styles/impact.ttf", fontSize)
        topTextSize = font.getsize(topString)
        bottomTextSize = font.getsize(bottomString)
...
async def cat_meeme(upper_text, lower_text, picture_name, endname):
    ...
    text_draw = Drawing()
    text_draw.font = join(getcwd(), "fridaybot/helpers/styles/impact.ttf")
